...
struct FeedbackOutput
{
    ...
    // false — ось выключена (например, реакция на руль не
    // распознана или инверсия не подтвердилась): рули этой оси
    // остаются на стиках пилота.
    bool axisEnabled[FeedbackConfig::AXIS_COUNT] = { true, true, true };

    // +1 — рули работают как в Config.h, -1 — обнаружена и принята
    // автоинверсия этой оси.
    int8_t axisSign[FeedbackConfig::AXIS_COUNT] = { 1, 1, 1 };
