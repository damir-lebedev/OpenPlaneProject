...
namespace FeedbackConfig
{
    ...
    // --------------------------------------------------------
    // Направление реакции и автоинверсия (ControlDirectionGuard)
    // --------------------------------------------------------

    // Разрешить автоинверсию. false — только сообщать и отключать ось.
    constexpr bool AUTO_INVERT_ENABLED = true;

    // Оценка эффективности должна уверенно держаться с обратным знаком
    // (модуль больше этого, в долях от априорной) дольше REVERSAL_CONFIRM_MS.
    constexpr float REVERSAL_EFFECTIVENESS_RATIO = 0.3f;
    constexpr uint32_t REVERSAL_CONFIRM_MS = 500;

    // Расходимость: команда держится в одну сторону, а ошибка угла за
    // окно выросла на столько градусов, и самолёт вращается против
    // команды быстрее DIVERGENCE_RATE_DPS.
    constexpr uint32_t DIVERGENCE_WINDOW_MS = 250;
    constexpr float DIVERGENCE_ERROR_GROWTH_DEG = 5.0f;
    constexpr float DIVERGENCE_RATE_DPS = 10.0f;
    constexpr float DIVERGENCE_MIN_COMMAND_US = 60.0f;

    // Анализ оси только когда пилот этим стиком почти не двигает.
    constexpr float STICK_QUIET_US = 50.0f;

    // После инверсии: за это время реакция должна стать правильной,
    // иначе инверсия отменяется.
    constexpr uint32_t INVERSION_VERIFY_MS = 1500;

    // Первые мс после инверсии не судить: серво ещё доходит, а самолёт
    // по инерции докручивается в прежнюю сторону.
    constexpr uint32_t INVERSION_GRACE_MS = 300;
