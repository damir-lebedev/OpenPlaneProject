...
namespace FeedbackConfig
{
    ...
    // --------------------------------------------------------
    // Ось работает наоборот -> выключить (ControlDirectionGuard).
    // Знак в полёте не переворачивается: направления определяются
    // на земле (калибровка установки IMU, проверка рулей).
    // --------------------------------------------------------

    // Оценка эффективности должна уверенно держаться отрицательной
    ...
    // Расходимость проверяется, только когда пилот этим стиком почти
    // не двигает.
