...
| `DebugConsole.h` | Команды в мониторе порта (`h`/`s`/`i`/`o`/`m`/`p`) |
...
| `ControlDirectionGuard.h` | Ось явно работает наоборот — выключить её (в полёте знак не переворачивается) |
...
**Ось работает наоборот.** Знаки осей в полёте **не переворачиваются**:
установка IMU определяется на земле (калибровка `o` + предполётная проверка
при включении), направления рулей — предполётной проверкой пилота. Переворот
в воздухе по косвенным признакам может ошибиться (порыв, срыв), и такая
ошибка стоит самолёта. `ControlDirectionGuard` — последняя страховка: если
оценка `b` уверенно отрицательная дольше 0.5 с или расходимость (коррекция
толкает к цели, а самолёт 250 мс вращается от цели, и ошибка выросла на 5°;
игнорируется, если оценка уверенно положительная — это порыв), ось
выключается до дизарма (`axisEnabled = false`, рули у пилота): бороться в
неправильную сторону хуже, чем не бороться. Отрицательная оценка `b` в
регулятор не идёт никогда.
...
| Перепутанный элерон, стабилизацию включили в полёте | Расходимость → ось выключена, крен больше не раскручивается |
| Перепутанный элерон, пилот качает крыльями в MANUAL | Найден по оценке `b` до включения стабилизации, ось выключена заранее |
| Болтанка 30 с | Ни одна ось не выключена зря |
