...
`test/test_feedback/test_main.cpp` (на ПК: `pio test -e native -f test_feedback`) — модель самолёта (независимые оси,
...
```bash
pio test -e native -f test_feedback      # на ПК, за секунды
